...
private fun printNextSteps(configFile: File, config: KnowledgeConfig) {
    ...
    println("  index patchlines    = ${config.indexPatchlines.joinToString(",")}")
    println("  enabled corpora     = ${config.enabledCorpora.joinToString(",")}")
    println("  docs sources       = ${config.docsSources.joinToString(",")}")
