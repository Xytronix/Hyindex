...
import com.hyve.knowledge.core.progress.StdoutProgressReporter
import com.hyve.knowledge.core.source.GitSourceProvider
import com.hyve.knowledge.core.version.RetentionPruner
...
private fun relaunchWithLargerHeapOrNull(args: Array<String>): Int? {
    if (System.getenv("HYVE_INDEXER_HEAP_OK") == "1") return null
...
private fun snapshotVersionIndex(versionDir: java.io.File, version: String, log: com.hyve.knowledge.core.logging.LogProvider) {
        ...
        val docRoots = buildList {
            if ("docs" in opts.corpora) {
                ...
                if ("support" in opts.docsSources)
                    runCatching { com.hyve.knowledge.core.source.SupportDocsSource.fetchInto(File(baseDir, "cache"), log, force = opts.force) }.getOrNull()?.let { add(it) }
                if ("blog" in opts.docsSources)
                    runCatching { com.hyve.knowledge.core.source.BlogDocsSource.fetchInto(File(baseDir, "cache"), log, force = opts.force) }.getOrNull()?.let { add(it) }
