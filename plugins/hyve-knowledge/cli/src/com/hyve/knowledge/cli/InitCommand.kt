...
data class InitArgs(
    ...
    companion object {
        val USAGE = """
            Usage: java -jar hyve-knowledge-indexer.jar init [options]
              ...
              --provider <name>        openai (default) | voyage | cohere | gemini | jina |
                                       ...
                                       local requires optional hyve-embeddings-local.jar
              ...
              --force                  Overwrite existing ~/.hyve/knowledge/mcp-config.json
              --help                   Show this help
        """.trimIndent()
...
private fun printNextSteps(configFile: File, config: KnowledgeConfig) {
    ...
    when (config.embeddingProvider.lowercase()) {
        "local" -> println("  - put hyve-embeddings-local.jar on the classpath (./gradlew :embeddings-local:shadowJar)")
        "ollama" -> {
    ...
    println("  - Index:  java -jar hyve-knowledge-indexer.jar --patchline release")
    println("  - Serve:  java -jar hyve-knowledge-mcp.jar")
}
