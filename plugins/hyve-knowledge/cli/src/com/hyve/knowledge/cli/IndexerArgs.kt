...
data class IndexerArgs(
    ...
    companion object {
        ...
        val USAGE = """
            ...
            Usage:
              java -jar hyve-knowledge-indexer.jar init [options]   # write default config
              java -jar hyve-knowledge-indexer.jar [options]        # build indexes
              java -jar hyve-knowledge-indexer.jar eval [options]   # run golden-set eval
            ...
            Examples:
              # First-time setup (writes ~/.hyve/knowledge/mcp-config.json)
              java -jar hyve-knowledge-indexer.jar init

              # Reindex only the pre-release patchline
              java -jar hyve-knowledge-indexer.jar --patchline pre-release

              # Reindex everything from scratch
              java -jar hyve-knowledge-indexer.jar --force

              # Only the code corpus for release
              java -jar hyve-knowledge-indexer.jar --patchline release --corpus code

              # Re-embed existing code in place after a model change (cheap; cache-aware)
              java -jar hyve-knowledge-indexer.jar --patchline all --corpus code --reembed
        """.trimIndent()
