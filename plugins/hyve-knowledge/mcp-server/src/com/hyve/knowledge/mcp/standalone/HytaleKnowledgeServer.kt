...
import com.hyve.knowledge.core.search.GraphTraversal
import com.hyve.knowledge.core.search.KnowledgeSearchService
import com.hyve.knowledge.core.search.ResponseEncoder
import com.hyve.knowledge.core.search.SearchResult
import com.hyve.knowledge.core.search.Snippets
import java.io.File
...
class HytaleKnowledgeServer(
    ...
    internal fun serviceForPatchline(patchline: String, warningOut: StringBuilder? = null): KnowledgeSearchService? {
        if ('@' !in patchline) {
            services[patchline]?.let { return it }
        ...
        val (line, version) = patchline.split('@', limit = 2)
    ...
    internal fun dbForPatchline(patchline: String, warningOut: StringBuilder? = null): KnowledgeDatabase? {
        if ('@' !in patchline) {
            databases[patchline]?.let { return it }
    ...
    internal fun sourceRootForPatchline(patchline: String): File? {
        val slug = if ('@' in patchline) {
            val (line, version) = patchline.split('@', limit = 2)
            VersionResolver.resolveSlug(basePath, line, version) ?: latestSlugs[line]
        } else {
            latestSlugs[patchline] ?: latestSlugs.values.firstOrNull()
        } ?: return null
            ...
            try {
                if (corpus == "all") {
                    val obj = buildJsonObject {
                        put("code", json.encodeToJsonElement(com.hyve.knowledge.core.search.IndexStats.serializer(), searchService.getCorpusStats(Corpus.CODE)))
                        put("gamedata", json.encodeToJsonElement(com.hyve.knowledge.core.search.IndexStats.serializer(), searchService.getCorpusStats(Corpus.GAMEDATA)))
                        put("docs", json.encodeToJsonElement(com.hyve.knowledge.core.search.IndexStats.serializer(), searchService.getCorpusStats(Corpus.DOCS)))
                        put("client", json.encodeToJsonElement(com.hyve.knowledge.core.search.IndexStats.serializer(), searchService.getCorpusStats(Corpus.CLIENT)))
                    }
