...
import com.hyve.knowledge.core.embedding.Reranker
...
class KnowledgeSearchServiceTest {
    ...
    fun `applyCorpusPenalty down-weights blog docs below an equal-scored repo doc`() {
        ...
        val adjusted = service.applyCorpusPenalty(mapped, com.hyve.knowledge.core.db.Corpus.DOCS)
    ...
    fun `applyCorpusPenalty leaves CODE corpus untouched`() {
        ...
        val adjusted = service.applyCorpusPenalty(mapped, com.hyve.knowledge.core.db.Corpus.CODE)
    ...
    fun `applyCorpusPenalty down-weights gamedata world nodes below a real item`() {
        ...
        val adjusted = service.applyCorpusPenalty(listOf(cave, item), com.hyve.knowledge.core.db.Corpus.GAMEDATA)
    ...
    fun `lexicalSearch finds a code node via camelCase split body`() {
        ...
        com.hyve.knowledge.index.FtsTokenizer.populate(
            db, "code",
            listOf(com.hyve.knowledge.index.FtsTokenizer.FtsRow(
                "code:Foo#registerPlayerEvent", "Foo#registerPlayerEvent", "void registerPlayerEvent() {}",
            )),
            splitBody = true,
        )
        val svc = KnowledgeSearchService(db, CorpusIndexManager(KnowledgeConfig()))
        val hits = svc.lexicalSearch("player", com.hyve.knowledge.core.db.Corpus.CODE, 10)
        assertEquals(1, hits.size)
    ...
    fun `lexicalSearch ranks a name-only match above a body-only match via column weights`() {
        ...
        com.hyve.knowledge.index.FtsTokenizer.populate(
            db, "gamedata",
            listOf(
                com.hyve.knowledge.index.FtsTokenizer.FtsRow("gamedata:in_name", "torch", "irrelevant text"),
                com.hyve.knowledge.index.FtsTokenizer.FtsRow("gamedata:in_body", "irrelevant", "a torch here"),
            ),
            splitBody = false,
        )
        val svc = KnowledgeSearchService(db, CorpusIndexManager(KnowledgeConfig()))
        val hits = svc.lexicalSearch("torch", com.hyve.knowledge.core.db.Corpus.GAMEDATA, 10)
    ...
    fun `lexicalSearch returns any-term match for a multi-word query`() {
        ...
        com.hyve.knowledge.index.FtsTokenizer.populate(
            db, "docs",
            listOf(com.hyve.knowledge.index.FtsTokenizer.FtsRow("docs:cmd", "Commands", "how to register a command")),
            splitBody = false,
        )
        val svc = KnowledgeSearchService(db, CorpusIndexManager(KnowledgeConfig()))
        val hits = svc.lexicalSearch("inventory command", com.hyve.knowledge.core.db.Corpus.DOCS, 10)
